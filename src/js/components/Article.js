...
// import CarouselControls from 'grommet/components/CarouselControls';
...
export default class Article extends Component {
  ...
  _getChildrenWidth(nodes) {
    if (!nodes) return 0;

    // Convert node array to real array.
    const childNodes = Array.prototype.slice.call(nodes, 0);
    let rowWidth = 0;

    childNodes.forEach((child)=> {
      const cssPosition = window.getComputedStyle(child,null).getPropertyValue('position');
      if (cssPosition !== 'fixed' && cssPosition !== 'absolute') {
        const rect = child.getBoundingClientRect();
        rowWidth = rowWidth + rect.width;
      }
    });
